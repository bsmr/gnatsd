...
func (s *Server) leafNodeAcceptLoop(ch chan struct{}) {
	...
	// Keep track of actual listen port.
	// This will be needed in case of config reload.
	// TODO(dlc) - Do reload logic.
	s.leafNodeActualPort = opts.LeafNode.Port
...
// Called when an inbound leafnode connection is either accepted.
...
func (c *client) processLeafUnsub(arg []byte) error {
	...
	// If we are routing add to the route map for the associated account.
	srv.updateRouteSubscriptionMap(acc, sub, -11)
	if updateGWs {
		srv.gatewayUpdateSubInterest(acc.Name, sub, -11)
	}
	// Now check on leafnode updates for other leaf nodes.
	srv.updateLeafNodes(acc, sub, -11)
	return nil
}
